...
namespace FreeDraw
{
  ...
  public class Drawable : MonoBehaviour
  {
    ...
    public static int Pen_Width = 13;
    ...
    [SerializeField] private GameObject _malang;
    ...
    void Update()
    {
      ...
      if (touched && !no_drawing_on_current_drag)
      {
        var touch = Input.GetTouch(0);
        // Convert mouse coordinates to world coordinates

        //float scaleFactor = GetComponent<Canvas>().scaleFactor;
        //Vector2 mouse_world_position = new Vector2(width / scaleFactor, height / scaleFactor);

        Vector2 mouse_world_position = Camera.main.ScreenToWorldPoint(touch.position);
        ...
        if (hit != null && hit.transform != null)
        {
          ...
          current_brush(mouse_world_position);                                                            //여기
      ...
      else if (!touched)
      {
        if (mouse_was_previously_held_down)
        {
          //이전에 클릭이 있었고 지금 터치없다면(=드래그 끝난거)
          //여기서 예측
          var predict_ch = gameObject.GetComponent<OcrModelPredict>().Predict();
          var result = OcrGameConfig.CheckNameChar(predict_ch);
          if (result && OcrGameConfig.AddNameIdx())
          {
            //정답이고 다음 글자가 있다면
            //캔버스 지우고 축하하기
            Debug.Log("다음글자 넘어가자");
            Celebrate();
            SaveSpriteToPng.SaveTextureToFile(drawable_sprite.texture, OcrGameConfig.GetFileName());
            StartCanvas();
          }
          else if (result)
          {
            Debug.Log("완전 성공이당");
            //정답인데 다음 글자가 없다면 완전성공임. 자체성공씬으로 이동
            //SceneManager.LoadScene("SuccessScene");
            Celebrate();
            SaveSpriteToPng.SaveTextureToFile(drawable_sprite.texture, OcrGameConfig.GetFileName());
            SceneManager.LoadScene("OcrSuccess");
          }
        }
        previous_drag_position = Vector2.zero;
        no_drawing_on_current_drag = false;
      }
      mouse_was_previously_held_down = touched;
    }
    ...
    public Vector2 WorldToPixelCoordinates(Vector2 world_position)
    {
      ...
      float pixelWidth = drawable_sprite.rect.width; //drawable_sprite.bounds.size.x;
      float pixelHeight = drawable_sprite.rect.height;
      float unitsToPixels = pixelWidth / drawable_sprite.bounds.size.x * transform.localScale.x;
      Debug.Log(unitsToPixels);
    ...
    {
      text.text = OcrGameConfig.GetNameChar().ToString();
      drawable = this;
    ...
    {
      if (_malang == null) return;
      _malang.GetComponent<AudioSource>().Play();
      _malang.GetComponent<EnemyAi>().currentState = SlimeAnimationState.Jump;

    }

    private void Awake()
    {
      Debug.Log("name : "+OcrGameConfig.name);
      StartCanvas();
    }
